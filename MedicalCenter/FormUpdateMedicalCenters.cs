...
{
    public partial class FormUpdateMedicalCenters : Form
    {
        private OleDbConnection dataConnection;
        private bool isManager;
        private int lastRow = 0;
        public FormUpdateMedicalCenters(OleDbConnection dataConnection, bool isManager)
        {
            InitializeComponent();
        ...
        private void FormUpdateMedicalCenters_Load(object sender, EventArgs e)
        {
        ...
        private void UpdateClick(object sender, EventArgs e)
        {
            try
            {
                OleDbCommand datacommand = new OleDbCommand();
                datacommand.Connection = dataConnection;
                datacommand.CommandText = "UPDATE tblMedicalCenter  \n" +
                                          "SET    medicalAddress    =  \"" + addressField.Text + "\" , \n" +
                                                  "medicalCity      =  \"" + cityCombo.Text + "\" , \n" +
                                                  "medicalPhone     =  \"" + phoneField.Text + "\" , \n" +
                                                  "medicalPicture   =  \"" + imageField.Text + "\" \n" +
                                          "WHERE  medicalName = \"" + nameField.Text + "\"";
                datacommand.ExecuteNonQuery();
                RefreshDataGridView();
                dataGridView1.CurrentCell = dataGridView1[0, lastRow];
                MessageBox.Show("Update tblMedicalCenter ended successfluly");
            }
            catch (Exception err)
            {
                MessageBox.Show("Update tblMedicalCenter failed \n" + err.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
