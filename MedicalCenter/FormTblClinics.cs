...
{
    public partial class FormTblClinics : Form
    {
        public FormTblClinics()
        {
            InitializeComponent();
        }

        private void FormTblClinics_Load(object sender, EventArgs e)
        {
        ...
        private void SaveButtonClick(object sender, EventArgs e)
        {
            try
            {
                DataSetClinics changes = (DataSetClinics)dataSetClinics.GetChanges();
                if (changes == null)
                    return;
                // Check for errors
                DataTable dt = changes.tblClinics.GetChanges();
                ...
                int numRows = tblClinicsTableAdapter.Update(changes);
                MessageBox.Show("Updated " + numRows + " rows", "Success");
                dataSetClinics.AcceptChanges();
            }
            catch (Exception ex)
            {
